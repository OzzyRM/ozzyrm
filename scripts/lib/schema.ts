...
import type { DocSchema, OrmDocgenAdapter, Parser, ReldocProjectConfig, ReldocSchemaSource } from "@reldoc/core";
import { isProjectConfig, postProcess } from "@reldoc/core";

export const DEFAULT_SCHEMA_VERSION = "1.0.0";

export function schemaFileLabel(source: ReldocSchemaSource): string {
    if (source.file) {
        return source.file;
    }
...
async function loadRawConfig(): Promise<ReldocProjectConfig> {
    ...
    return {
        output: config.output ?? "./web/schemas",
        schemas: [{
            id: config.orm,
            label: config.orm,
            ...config,
...
async function generateOne(source: ReldocSchemaSource): Promise<GeneratedSchemaEntry> {
...
export async function loadConfig(): Promise<ReldocProjectConfig> {
    return loadRawConfig();
}

export async function generateAllSchemas(): Promise<string[]> {
    const config = await loadRawConfig();
    const outputDir = resolve(process.cwd(), config.output ?? "./web/schemas");
...
export async function getWatchedPaths(): Promise<string[]> {
    const config = await loadRawConfig();
    const paths: string[] = [resolve(process.cwd(), "reldoc.config.ts")];

    for (const source of config.schemas) {
        const resolved = source.include.map((path) => resolve(process.cwd(), path));

        if (source.orm === "prisma") {
            const { expandPrismaWatchPaths } = await import("@reldoc/parser-prisma");
