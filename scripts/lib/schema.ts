...
import type { DocSchema, OrmDocgenAdapter, Parser, OzzyRMProjectConfig, OzzyRMSchemaSource } from "@ozzyrm/core";
import { isProjectConfig, postProcess } from "@ozzyrm/core";
...
export async function loadParser(orm: OrmDocgenAdapter["orm"]): Promise<Parser> {
    switch (orm) {
        ...
        case "drizzle": {
            const { DrizzleParser } = await import("@ozzyrm/parser-drizzle");
...
export async function getWatchedPaths(): Promise<string[]> {
    ...
    for (const source of config.schemas) {
        ...
        if (source.orm === "prisma") {
            const { expandPrismaWatchPaths } = await import("@ozzyrm/parser-prisma");
            paths.push(...await expandPrismaWatchPaths(resolved));
            continue;
        }

        if (source.orm === "drizzle") {
            const { expandDrizzleWatchPaths } = await import("@ozzyrm/parser-drizzle");
